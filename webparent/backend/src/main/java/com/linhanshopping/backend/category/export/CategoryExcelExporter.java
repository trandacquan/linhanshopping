...
// Xuất danh sách Category ra file .xlsx, dùng thư viện Apache POI.
// Cấu trúc: workbook (file) -> sheet (1 trang tính) -> row (hàng) -> cell (ô)
public class CategoryExcelExporter extends AbstractExporter {
	...
	public CategoryExcelExporter() {
		workbook = new XSSFWorkbook(); // khởi tạo workbook rỗng đầu tiên
	}

	// Hàm dùng chung để tạo 1 ô: chọn row, cột, giá trị và style tương ứng
	private void createCell(XSSFRow row, int columnIndex, Object value, CellStyle style) {
		XSSFCell cell = row.createCell(columnIndex);
		sheet.autoSizeColumn(columnIndex); // tự co giãn độ rộng cột theo nội dung

		// Excel cell cần set giá trị đúng kiểu (Integer/Boolean/String) chứ không dùng chung 1 hàm setCellValue
	...
	// Tạo hàng tiêu đề (header) ở hàng số 0, có bold + gạch chân
	...
	// Ghi từng category thành 1 hàng, bắt đầu từ hàng số 1 (hàng 0 đã dùng làm header)
	...
	public void export(List<Category> listCategories, HttpServletResponse response) throws IOException {
		...
		workbook.write(outputStream); // ghi toàn bộ workbook ra response để trình duyệt tải về
		workbook.close();
		outputStream.close();
	}
