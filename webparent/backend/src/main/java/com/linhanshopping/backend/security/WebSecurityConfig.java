...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public DaoAuthenticationProvider authenticationProvider() {
		...
		// gán đối tượng shoppingUserDetailsService -->khi user bấm submit form login nó
		// sẽ vào đối tượng này để kiểm tra email và password
		authProvider.setUserDetailsService(userDetailsService());
		// gán đối tượng bCryptPasswordEncoder -->nó sẽ so sánh password sau khi giải mã
		// có giống với password user nhập hay ko, nếu giống thì dăng nhập thành công
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/users/**").hasAnyAuthority("Admin")
				// muốn gửi request /users/** đến controller thì phải đăng nhập(xác thực) thành
				// công và có role Admin(phân quyền)
				...
				.formLogin()
					...
					.usernameParameter("email")// <input type="email" name="email" /> -->khai báo giá trị của thuộc tính
											// name
					.permitAll()// có thể gửi request /login mà ko cần đăng nhập
				.and().logout().permitAll()
				.and()
					.rememberMe()// khi tắt server thì user ko cần đăng nhập lại
						.key("AbcDefgHijKlmnOpqrs_1234567890")// key là chuỗi bất kỳ, khi user đăng nhập thành công và có
														// checked rememberMe,
														// nó sẽ lấy chuỗi này mã hóa và lưu vào trong cookies, đó là lý do user ko cần
														// đăng nhập lại,
														// khi key hết hạn hoặc server thay đổi chuỗi key thì user phải đăng nhập lại
	...
	public void configure(WebSecurity web) throws Exception {
		...
		// có thể truy cập các folder images, folder js, folder webjars mà ko cần đăng
		// nhập
	}
